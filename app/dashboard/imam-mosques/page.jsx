        ...
        <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <form onSubmit={submitMosque} className="space-y-3">
                        <input value={mosqueForm.name} onChange={handleMosqueFormChange("name")} placeholder="Mosque Name" className="w-full border border-gray-200 rounded-lg px-3 py-2" />
                        <input value={mosqueForm.contact_person} onChange={handleMosqueFormChange("contact_person")} placeholder="Contact Person" className="w-full border border-gray-200 rounded-lg px-3 py-2" />
                        <select value={mosqueForm.city} onChange={handleMosqueFormChange("city")} className="w-full border border-gray-200 rounded-lg px-3 py-2">
                            <option value="">Select City</option>
                            {cities.map((city) => (
                                <option key={city.id} value={city.id}>{city.name}</option>
                            ))}
                        </select>
                        <textarea value={mosqueForm.address} onChange={handleMosqueFormChange("address")} placeholder="Address" className="w-full border border-gray-200 rounded-lg px-3 py-2" rows={3} />
                        <input value={mosqueForm.phone} onChange={handleMosqueFormChange("phone")} placeholder="Phone" className="w-full border border-gray-200 rounded-lg px-3 py-2" />
                        <input value={mosqueForm.email} onChange={handleMosqueFormChange("email")} placeholder="Email" className="w-full border border-gray-200 rounded-lg px-3 py-2" />
                ...
                <div className="bg-white rounded-2xl border border-gray-100 p-6">
                    ...
                    <div className="space-y-3">
                        {mosques.map((mosque) => (
                            <div key={mosque.id} className={`border rounded-xl p-3 ${selectedMosqueId === mosque.id ? "border-[#238B57]" : "border-gray-200"}`}>
                                <button
                                    type="button"
                                    onClick={() => setSelectedMosqueId(mosque.id)}
                                    className="text-left w-full"
                                >
                                    <h3 className="font-semibold text-slate-800">{mosque.name}</h3>
                                    <p className="text-xs text-slate-500">{mosque.address}</p>
                                ...
                                <div className="flex gap-2 mt-3">
                                    <button onClick={() => startEditMosque(mosque)} className="text-xs px-3 py-1 rounded bg-[#EEF7FB] text-[#1F6F8B]">Edit</button>
                                    <button onClick={() => deleteMosque(mosque.id)} className="text-xs px-3 py-1 rounded bg-red-50 text-red-600">Delete</button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <form onSubmit={submitTimetableEntry} className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
                    <input type="time" value={timetableForm.fajr_adhan} onChange={handleTimetableFormChange("fajr_adhan")} className="border border-gray-200 rounded-lg px-3 py-2" />
                    <input type="time" value={timetableForm.fajr_iqamah} onChange={handleTimetableFormChange("fajr_iqamah")} className="border border-gray-200 rounded-lg px-3 py-2" />
                    <input type="time" value={timetableForm.dhuhr_adhan} onChange={handleTimetableFormChange("dhuhr_adhan")} className="border border-gray-200 rounded-lg px-3 py-2" />
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {timetableRows.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="px-3 py-4 text-slate-500">No timetable entry for selected month.</td>
                                </tr>
                            )}
